...
function resolveShippingCollected(status, providedRaw, remaining, wasDispatched, previouslyReceived) {
    if (status !== 'تم الصرف') {
...
router.put('/:id', auth_1.requireManager, async (req, res) => {
    try {
        ...
        const sale = await prisma_1.default.$transaction(async (tx) => {
            ...
            if (wasReservation) {
                await adjustReserved(extractModels(oldSale), -1, seasonId, tx);
            }
        ...
        (0, auditHelper_1.logAudit)({ user: req.user, module: 'Sales', action, record_id: id,
...
router.post('/:id/convert-reservation', auth_1.requireManager, async (req, res) => {
    try {
        ...
        const resolved = resolveShippingCollected('تم الصرف', req.body?.received_amount, sale.remaining, false, 0);
        if ('error' in resolved) {
            return res.status(400).json({ message: resolved.error });
        }
        ...
        (0, auditHelper_1.logAudit)({ user: req.user, module: 'Sales', action: 'SHIPPING_PAYOUT_CONFIRMED', record_id: id,

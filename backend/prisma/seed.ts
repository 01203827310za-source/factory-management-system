...
async function main() {
  ...
  if (stockCount === 0) {
    await prisma.readyStock.createMany({
      data: [
        { model_code: 'M001', product_name: 'تيشيرت قطن',  color: 'أبيض', opening_balance: 50, cost_per_piece: 45,  location: 'المخزن الرئيسي' },
        { model_code: 'M001', product_name: 'تيشيرت قطن',  color: 'أسود', opening_balance: 30, cost_per_piece: 45,  location: 'المخزن الرئيسي' },
  ...
  if (expCount === 0) {
    await prisma.expenseRevenue.createMany({
      data: [
        { date: '2025-01-01', operation_type: 'راس مالى',       statement: 'رأس المال المبدئى',  amount_in: 100000, amount_out: 0 },
      ...
      data: [
        {
          ...
          invoice_value: 725, deposit_paid: 300, deposit_receiver: '', remaining: 425,
          shipping_number: 'SHP-001', order_status: 'لم يتم الصرف', delivery_method: 'ايرجنت',
          mobile: '01012345678', warehouse: 'المخزن الرئيسي', shipping_collected: 0,
        },
        {
        ...
        },
        {
          ...
          invoice_value: 1560, deposit_paid: 500, deposit_receiver: '', remaining: 1060,

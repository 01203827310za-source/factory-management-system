...
import { debtsStore, debtPaymentsStore } from '../data/store';
...
export default function Debts() {
  ...
  // Edit payment modal
  const [editPay, setEditPay] = useState<DebtPayment | null>(null);
  const [editPayForm, setEditPayForm] = useState({ date: '', amount: 0, payment_method: '', description: '' });

  // Delete payment confirm
  const [deletePayConfirm, setDeletePayConfirm] = useState<{ debtId: number; pid: number } | null>(null);

  const loadData = async () => {
    setLoading(true);
    try { setItems(await debtsStore.getAll()); }
  ...
  const openEdit = (item: Debt) => {
    setEditItem(item);
    setForm({ date: item.date, name: item.name, total_amount: item.total_amount, amount_paid: item.amount_paid });
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      if (editItem) { await debtsStore.update(editItem.id, form); toast('success', 'تم التعديل'); }
      else { await debtsStore.add(form); toast('success', 'تم الإضافة'); }
  ...
  const handleDelete = async (id: number) => {
    try { await debtsStore.remove(id); toast('success', 'تم الحذف'); setDeleteConfirm(null); await loadData(); }
    catch (e: unknown) { toast('error', e instanceof Error ? e.message : 'خطأ'); }
  };

  // ----- Payment handlers -----
  const openPayModal = (item: Debt) => {
    setPayTarget(item);
  ...
  const handleAddPayment = async () => {
    if (!payTarget) return;
    if (!payForm.receiver) { toast('error', 'يرجى تحديد من حول المبلغ'); return; }
    if (payForm.amount <= 0) { toast('error', 'يرجى إدخال مبلغ صحيح'); return; }
    setSaving(true);
    try {
      await debtPaymentsStore.add(payTarget.id, payForm);
      toast('success', `تم تسجيل دفعة ${payForm.amount.toLocaleString('ar-EG')} من ${payForm.receiver}`);
      setPayTarget(null);
  ...
  const openEditPay = (p: DebtPayment) => {
    setEditPay(p);
  ...
  const handleEditPayment = async () => {
    ...
    setSaving(true);
    try {
      await debtPaymentsStore.update(editPay.debt_id, editPay.id, editPayForm);
  ...
  const handleDeletePayment = async () => {
    ...
    try {
      await debtPaymentsStore.remove(deletePayConfirm.debtId, deletePayConfirm.pid);
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(items.map(r => ({
      'التاريخ': r.date, 'الاسم': r.name, 'الإجمالي': r.total_amount,
      'المدفوع': r.amount_paid, 'المتبقي': r.remaining,
    })));
    ...
    <div className="space-y-5">
      ...
      {totalRemaining > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center justify-between">
          <span className="font-medium text-red-700">إجمالي المتبقي من الديون</span>
          <span className="text-xl font-bold text-red-700">{totalRemaining.toLocaleString('ar-EG')}</span>
        </div>
      )}

      <div className="relative max-w-md">
        <Search size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" />
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="بحث بالاسم..." className={`w-full pr-10 ${ic}`} />
      </div>
          ...
          <table className="w-full text-sm whitespace-nowrap">
            <thead>
              <tr className="bg-[#1e3a5f] text-white">
                {['#','التاريخ','الاسم','الإجمالي','المدفوع','المتبقي','الحالة','إجراءات'].map(h =>
            ...
            <tbody>
              ...
              {filtered.map((item, idx) => (
                <Fragment key={item.id}>
                  <tr className={`border-t border-gray-100 hover:bg-blue-50/40 transition ${item.remaining > 0 ? 'bg-red-50/30' : ''}`}>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${item.remaining === 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                        {item.remaining === 0 ? 'مسدد' : 'متبقي'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                    ...
                    <tr className="border-t border-gray-100">
                      <td colSpan={8} className="bg-slate-50 px-6 py-4">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <span className="text-sm font-bold text-slate-700">سجل سداد الدين</span>
                                  ...
                                  <tr key={p.id} className="border-t border-slate-100 hover:bg-slate-100 transition bg-white">
                                    ...
                                    <td className="px-3 py-2 text-center">
                                      <div className="flex items-center justify-center gap-1">
                                        ...
                                        <button onClick={() => setDeletePayConfirm({ debtId: item.id, pid: p.id })} className="p-1 text-red-600 hover:bg-red-100 rounded"><Trash2 size={12} /></button>
                                      </div>
                                    </td>
                                  </tr>
                                ))}
      ...
      {/* Add Payment Modal */}
      {payTarget && (
        <Modal isOpen={true} onClose={() => setPayTarget(null)} title={`تسجيل دفعة — ${payTarget.name}`}>
          <div className="space-y-3">
            <div className="bg-gray-50 rounded-lg p-3 text-sm">
              <span className="text-gray-600">المتبقي: </span>
              <span className="font-bold text-red-600">{payTarget.remaining.toLocaleString('ar-EG')}</span>
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><label className={lc}>المبلغ *</label><input type="number" className={ic} value={payForm.amount} onChange={e => setPayForm({...payForm, amount: parseFloat(e.target.value)||0})} min={0} max={payTarget.remaining} /></div>
              ...
              <div><label className={lc}>من حول المبلغ *</label>
      ...
      <Modal isOpen={deleteConfirm !== null} onClose={() => setDeleteConfirm(null)} title="تأكيد الحذف" size="sm">
        <p className="text-gray-600">هل أنت متأكد من حذف هذا الدين؟</p>
